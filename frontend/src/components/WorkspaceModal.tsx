        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input autoFocus type="text" value={name} onChange={e => setName(e.target.value)}
                    ...
                    className="w-full bg-white/5 border border-white/5 focus:border-brand-accent/50 rounded-2xl py-4 pl-12 pr-6 text-sm font-medium outline-none transition-all"
                    required />
                </div>
              </div>
              ...
              <button type="submit"
                className="w-full bg-brand-accent hover:brightness-110 text-white font-black py-4 rounded-2xl shadow-[0_8px_20px_rgba(139,92,246,0.3)] transition-all flex items-center justify-center gap-2">
                <Server className="w-5 h-5" />
                Add Server
              </button>
            </form>
